...
### Approach
My goal for the technical assesment was to utilize test driven development to build a flexible React app. I spread my work over a few days, and I broke things up into stages: planning, testing, and implementation.
...
Once I had the path my data would take, I built my tests and functions. First, the hotel-sort.service, used .sort() and a switch statement to reorder the input array based on the user's input (or lack of input). Once the function passed the tests, I moved onto the filter function in hotel-filter.service. Using a combination of built in methods on top of .filter(), I built a function that successfully passed the second battery of tests.

I used state variables to capture user input and pass their search terms into the filter and sort functions in the SearchControls component. React's ability to dynamically rerender the DOM was the backbone of my solution. When the user changes an input value, React dynamically responds and calls the sort/filter functions. If the user does not sort/filter the hotels, the array from the API will pass through both functions unchanged. The results pass through to the HoteList and are rendered on the DOM. If the user's filter matches nothing from the API call OR if the API call fails, a component renders that asks them to refresh their browswer.
